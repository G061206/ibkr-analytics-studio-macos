...
const ENGLISH_UI_REPLACEMENTS = [
  ["主推通过桌面应用自动拉取 Activity Flex Query，不需要手动下载 CSV。", "The desktop app can fetch the Activity Flex Query automatically, so you do not need to download CSV files manually."],
  ["Token 和 Query ID 只保存在本机浏览器存储，并只发送到 IBKR Flex Web Service。", "Token and Query ID are saved only in local browser storage and sent only to IBKR Flex Web Service."],
  ["Flex API 自动拉取仅在桌面应用中可用。", "Automatic Flex API fetching is available only in the desktop app."],
        ...
        <section class="upload-grid">
          <div class="upload-stack">
            <section class="info-card flex-card is-primary">
              ...
              <p class="card-kicker">主推通过桌面应用自动拉取 Activity Flex Query，不需要手动下载 CSV。</p>
              <div class="flex-form">
                <label>
                ...
                <p class="flex-status">${escapeHtml(state.flexStatus || (canUseNativeFlex() ? "Token 和 Query ID 只保存在本机浏览器存储，并只发送到 IBKR Flex Web Service。" : "Flex API 自动拉取仅在桌面应用中可用。"))}</p>
              </div>
            </section>
...
function getNativeBridge() {
  if (window.ibkrNative?.postMessage && window.ibkrNative?.addEventListener && window.ibkrNative?.removeEventListener) {
    return window.ibkrNative;
  }
...
function addNativeMessageListener(handler) {
  getNativeBridge()?.addEventListener("message", handler);
}

function removeNativeMessageListener(handler) {
  getNativeBridge()?.removeEventListener("message", handler);
}

function postNativeMessage(payload) {
  getNativeBridge()?.postMessage(payload);
}
...
function checkForUpdates({ manual = false } = {}) {
  if (!canUseNativeFlex()) {
    ...
    state.updateError = "Update checks are available only in the desktop app.";
    render();
    return;
  }
  ...
  const handleMessage = (event) => {
    ...
    removeNativeMessageListener(handleMessage);
  ...
  addNativeMessageListener(handleMessage);
  postNativeMessage({
